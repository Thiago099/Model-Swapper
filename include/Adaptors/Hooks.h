...
void Hooks::MoveItemHooks<RefType>::pickUpObject(RefType* a_this, RE::TESObjectREFR* a_object, int32_t a_count,
    ...
    EventHandler::GetSingleton()->OnItemPickup(a_this,a_object,a_count);
                                                               ...
                                                               RE::TESObjectREFR* a_move_to_ref, const RE::NiPoint3* a_drop_loc, const RE::NiPoint3* a_rotate)
{
	...
	auto manager = EventHandler::GetSingleton();

	if (a_move_to_ref) {
        manager->OnItemTransfer(a_this, a_item, a_count, a_move_to_ref);
	}
	else {
        manager->OnItemDrop(a_reason, a_this, a_item, a_count);
	}
