...
int32_t Variants::pickVariant(const models& models, const char* str, const uint32_t seed) {
...
variant* Variants::getVariant(const models& models, const char* str, const uint32_t variant) {
    const auto key = Str::processString(str);
    if (const auto it = models.find(key); it != models.end()) {
